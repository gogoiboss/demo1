...
def test_midnight_crossing():
    ...
    for row in df_derived.itertuples():
        print(f"Seq {row.seq}: Arr Day {row.arr_day_of_journey}, Dep Day {row.dep_day_of_journey} | "
    ...
    assert seq2.arr_day_of_journey == 2, f"Expected arr_day_of_journey=2, got {seq2.arr_day_of_journey}"
    assert seq2.section_run_time.total_seconds() == 25 * 60, f"Expected run time 25 mins, got {seq2.section_run_time}"
