...
def _evaluate_slice(
    route_df: pd.DataFrame, features: list[str]
    ...
    engine.fit(
        train[features],
        train["actual_delay_minutes"],
        calibration[features],
        calibration["actual_delay_minutes"],
    )
    ...
    prediction_rows = test[
        ["train_number", "journey_date", "actual_delay_minutes", "prior_leg_delay"]
    ].copy()
    ...
    metrics = {
        ...
        "mae_improvement_min": round(
            float(
                mean_absolute_error(y_true, naive)
                - mean_absolute_error(y_true, full_p50)
            ),
            3,
        ),
        ...
        "coverage_90_pct": round(
            float(prediction_rows["covered_90_interval"].mean() * 100), 2
        ),
...
def run_backtest(
    ...
    for route in routes:
        ...
        if route_df.empty:
            ...
            continue
        availability.append(
            {"route": str(route), "available": True, "n_rows": int(len(route_df))}
        )
    ...
    baseline_mae = mean_absolute_error(
        all_test["actual_delay_minutes"], all_test["prior_leg_delay"]
    )
    full_mae = mean_absolute_error(
        all_test["actual_delay_minutes"], all_test["xgboost_p50_delay_min"]
    )
    overall = {
        ...
        "mae_improvement_pct": round(
            float((baseline_mae - full_mae) / baseline_mae * 100), 2
        ),
        "coverage_90_pct": round(
            float(all_test["covered_90_interval"].mean() * 100), 2
        ),
        "avg_interval_width_min": round(
            float((all_test["p90_delay_min"] - all_test["p10_delay_min"]).mean()), 3
        ),
        "conflict_adjustment_rows": int(
            all_test["conflict_adjustment_min"].ne(0).sum()
        ),
    }
    representative = all_test.loc[
        (all_test["actual_delay_minutes"] - all_test["xgboost_p50_delay_min"])
        .abs()
        .idxmax()
    ]
    ...
    real_12301 = (
        route_12301.loc[route_12301["actual_delay_minutes"].idxmax()]
        if not route_12301.empty
        else representative
    )
    example = {
        ...
        "full_pipeline_p50_delay_min": round(
            float(representative["xgboost_p50_delay_min"]), 3
        ),
    ...
    real_12301_example = {
        ...
        "full_pipeline_p50_delay_min": round(
            float(real_12301["xgboost_p50_delay_min"]), 3
        ),
