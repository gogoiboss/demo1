...
def main():
    ...
    shap_values = explainer(
        X_train.iloc[:200]
    )  # Run on a subset to avoid extreme slowness
    ...
    if "prior_leg_delay" in df.columns and "actual_delay_minutes" in df.columns:
        ...
        for date, group in df.groupby(df["journey_date"].dt.date):
            if len(group) > 50:
                daily_corrs.append(
                    group["prior_leg_delay"].corr(group["actual_delay_minutes"])
                )
        ...
        print(
            f"Global Pearson Correlation (Rake Lateness -> Next Leg): {corr_global:.3f}"
        )
        print(
            f"Intra-Day Avg Correlation (Weather/Storm confound check): {avg_daily_corr:.3f}"
        )
        print(
            "Train/Test Split Methodology: Chronological TimeSeriesSplit (No Shuffling)"
        )
        print("=============================================\n")
