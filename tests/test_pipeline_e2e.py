...
def test_real_pipeline_runs_from_raw_data_to_calibrated_output():
    ...
    assert result["provenance"]["fit_policy"] == "calibration_model_fit_on_chronological_training_split"
    assert result["provenance"]["git_commit"] != ""


def test_pipeline_graph_stage_calls_real_conflict_detector():
    pipeline = RippleETAPipeline(graph=build_timed_event_graph(SCHEDULES_DEMO, min_headway=10.0))
    state = pd.DataFrame([
        {"train_id": "12301", "station": "KANPUR", "event_type": "dep", "delay_min": DELAYS_DEMO["12301__KANPUR__dep"]},
        {"train_id": "56789", "station": "KANPUR", "event_type": "dep", "delay_min": DELAYS_DEMO["56789__KANPUR__dep"]},
    ])
