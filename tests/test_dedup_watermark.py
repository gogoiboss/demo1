...
from src.ingestion.railradar_client import get_live_status, _WATERMARKS, _PROCESSED_PINGS, _VALIDATION_METRICS
...
def test_dedup_and_watermark():
    ...
    with mock.patch("src.ingestion.railradar_client._get", return_value=ping_1), \
    ...
    with mock.patch("src.ingestion.railradar_client._get", return_value=ping_new), \
         mock.patch("src.ingestion.railradar_client.RIPPLEETA_MODE", "live"):
    ...
    with mock.patch("src.ingestion.railradar_client._get", return_value=ping_stale), \
         mock.patch("src.ingestion.railradar_client.RIPPLEETA_MODE", "live"):
