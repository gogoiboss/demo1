...
def run_backtest(data_path: str | Path = "data/processed/kaggle_competition_cleaned.parquet") -> dict[str, float | int | str]:
    path = Path(data_path)
    if not path.exists():
        raise FileNotFoundError(f"Historical artifact not found: {path}")

    frame = engineer_all_features(pd.read_parquet(path)).dropna(subset=DEFAULT_FEATURES + ["actual_delay_minutes"])
    ...
    engine, _ = train_and_calibrate(train_frame, features=DEFAULT_FEATURES, save_path=None)
...
def main() -> None:
    ...
    parser.add_argument("--data", default="data/processed/kaggle_competition_cleaned.parquet")
